...
`daemon install` writes the service with your current `PATH`. Agents use their own stored logins; nothing else is copied. The service is named by the vault's id, which each vault makes the first time it needs one and never shares: `io.dreams.vault-<id>` on macOS and `dreams-vault-<id>.service` on Linux. Thus each vault gets its own service, even two vaults with the same file name, and a vault that you move keeps its name. `install` prints the name. Logs go to `~/Library/Logs/dreams/vault-<id>.log` on macOS and to `journalctl --user -u dreams-vault-<id>` on Linux.
